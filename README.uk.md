...
Стабільний **рушій** (`engine.py`) рендерить активний **декларативний маніфест** (профіль) на кожному кадрі. Перемикання профілю — це зміна вказівника, не регенерація. Маніфест резервує секцію `rules:` під майбутнє автоперемикання за контекстом (ще не збудовано). Дизайн: [SPEC.md](SPEC.md). Пояснення показників: [CATALOG.md](CATALOG.md).
